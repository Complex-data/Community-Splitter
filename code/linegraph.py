...
class LINEGraph(object):
    ...
    def read_adj_to_graph(self, adj_train):
        self.G = nx.DiGraph()
        for src in range( adj_train.shape[0]): 
            for dst in range( adj_train.shape[0]):
                if adj_train[src][dst]==1:
    ...
    #  def read_linegraph(self, g_graph, weighted=False):
    #      self.G = nx.DiGraph()
