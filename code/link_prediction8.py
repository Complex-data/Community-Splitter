            ...
            while len(listr) < (nump+1)/2:
                ...
                for i in range(0, len(listp)):
                    for j in range(i,len(listp)):
                        if i != j:
                            ij_emb = np.dot(p_emb_matrix[listp[i]], p_emb_matrix[listp[j]])
                            if ij_emb > max_emb:
                                max_emb = ij_emb
                                max_x = i
                                max_y = j
                listr.append(listp[max_x])
                listr.append(listp[max_y])
                del listp[max_x]
                del listp[max_y-1]
...
def calculate_all_scores(g_name, adj_sparse, features_matrix=None, directed=False, \
    ...
    tf.compat.v1.set_random_seed(random_state)  # Consistent GAE training
    ...
    if verbose >= 1:
        ...
        print ('line (Edge Embeddings) Validation ROC score: ', str(line_calculate['val_roc']))
        print ('line (Edge Embeddings) Validation AP score: ', str(line_calculate['val_ap']))
    ...
    if verbose >= 1:
        ...
        print ('GraphGAN Validation ROC score: ', str(graphgan_scores['val_roc']))
        print ('GraphGAN Validation AP score: ', str(graphgan_scores['val_ap']))
    ...
    # Using bootstrapped edge embeddings + logistic regression
    splitBIgru_scores_res = splitBIgru_scores(g_name, edgelist_path, train_test_split, embedding_dim=embedding_dim,
             walk_length=walk_length,
             num_walks_node=num_walks_node,
             constraint_learning_rate_scaling_factor=constraint_learning_rate_scaling_factor,
             iterations=iterations,
    ...
    lp_scores['splitBIgru_emb'] = splitBIgru_scores_res
    ...
    # Using bootstrapped edge embeddings + logistic regression
    splitBIlstm_scores_res = splitBIlstm_scores(g_name, edgelist_path, train_test_split, embedding_dim=embedding_dim,
             walk_length=walk_length,
             num_walks_node=num_walks_node,
             constraint_learning_rate_scaling_factor=constraint_learning_rate_scaling_factor,
             iterations=iterations,
             ...
             num_layers=num_layers,
             hidden_size=hidden_size)
    lp_scores['splitBIlstm_emb'] = splitBIlstm_scores_res




    ##-------------------------------splitter+Community-lcmi----------------------#
    edgelist_path = '/home/hxm/persona/data/edgelist' + g_name + '.edgelist'
    nx.write_edgelist(g_train, edgelist_path, data=False)
    if lp_scores['splitBIgru_emb']['test_roc'] > lp_scores['splitBIlstm_emb']['test_roc']:
        method = 'BIgru'
    else:
        method = 'BIlstm'
    if lp_scores['splitter']['test_roc'] > lp_scores['splitBIlstm_emb']['test_roc'] and lp_scores['splitter']['test_roc'] > lp_scores['splitBIgru_emb']['test_roc']:
        method = 'splitter'
    # Using bootstrapped edge embeddings + logistic regression
    splitCommunity_scores_res = splitCommunity_scores(g_name, edgelist_path, train_test_split, embedding_dim=embedding_dim,
                                              walk_length=walk_length,
                                              num_walks_node=num_walks_node,
                                              constraint_learning_rate_scaling_factor=constraint_learning_rate_scaling_factor,
                                              iterations=iterations,
                                              ...
                                              window_size=window_size,
                                              num_layers=num_layers,
                                              hidden_size=hidden_size*2,    # *2加强特征表达
                                              local_clustering_fn='lcmi',
                                              method=method)
    lp_scores['splitCommunity-lcmi_emb'] = splitCommunity_scores_res
    '''
    ##-------------------------------splitter+Community-lc----------------------#
    edgelist_path = '/home/hxm/persona/data/edgelist' + g_name + '.edgelist'
    nx.write_edgelist(g_train, edgelist_path, data=False)
    if lp_scores['splitBIgru_emb']['test_roc'] > lp_scores['splitBIlstm_emb']['test_roc']:
        method = 'BIgru'
    ...
    splitCommunity_scores_res = splitCommunity_scores(g_name, edgelist_path, train_test_split, embedding_dim=embedding_dim,
                                              ...
                                              num_layers=num_layers,
                                              hidden_size=hidden_size,
                                              local_clustering_fn='lc',
                                              method=method)
    lp_scores['splitCommunity-lc_emb'] = splitCommunity_scores_res

    ##-------------------------------splitter+Community-lm----------------------#
    edgelist_path = '/home/hxm/persona/data/edgelist' + g_name + '.edgelist'
    nx.write_edgelist(g_train, edgelist_path, data=False)
    if lp_scores['splitBIgru_emb']['test_roc'] > lp_scores['splitBIlstm_emb']['test_roc']:
        method = 'BIgru'
    ...
    splitCommunity_scores_res = splitCommunity_scores(g_name, edgelist_path, train_test_split, embedding_dim=embedding_dim,
                                              ...
                                              num_layers=num_layers,
                                              hidden_size=hidden_size,
                                              local_clustering_fn='lm',
                                              method=method)
    lp_scores['splitCommunity-lm_emb'] = splitCommunity_scores_res

    ##-------------------------------splitter+Community-li----------------------#
    edgelist_path = '/home/hxm/persona/data/edgelist' + g_name + '.edgelist'
    nx.write_edgelist(g_train, edgelist_path, data=False)
    if lp_scores['splitBIgru_emb']['test_roc'] > lp_scores['splitBIlstm_emb']['test_roc']:
        method = 'BIgru'
    ...
    splitCommunity_scores_res = splitCommunity_scores(g_name, edgelist_path, train_test_split, embedding_dim=embedding_dim,
                                              ...
                                              num_layers=num_layers,
                                              hidden_size=hidden_size,
                                              local_clustering_fn='li',
                                              method=method)
    lp_scores['splitCommunity-li_emb'] = splitCommunity_scores_res

    ##-------------------------------splitter+Community-cm----------------------#
    edgelist_path = '/home/hxm/persona/data/edgelist' + g_name + '.edgelist'
    nx.write_edgelist(g_train, edgelist_path, data=False)
    if lp_scores['splitBIgru_emb']['test_roc'] > lp_scores['splitBIlstm_emb']['test_roc']:
        method = 'BIgru'
    ...
    splitCommunity_scores_res = splitCommunity_scores(g_name, edgelist_path, train_test_split, embedding_dim=embedding_dim,
                                              ...
                                              num_layers=num_layers,
                                              hidden_size=hidden_size,
                                              local_clustering_fn='cm',
                                              method=method)
    lp_scores['splitCommunity-cm_emb'] = splitCommunity_scores_res


    ##-------------------------------splitter+Community-ci----------------------#
    edgelist_path = '/home/hxm/persona/data/edgelist' + g_name + '.edgelist'
    nx.write_edgelist(g_train, edgelist_path, data=False)
    if lp_scores['splitBIgru_emb']['test_roc'] > lp_scores['splitBIlstm_emb']['test_roc']:
        method = 'BIgru'
    ...
    splitCommunity_scores_res = splitCommunity_scores(g_name, edgelist_path, train_test_split, embedding_dim=embedding_dim,
                                              ...
                                              num_layers=num_layers,
                                              hidden_size=hidden_size,
                                              local_clustering_fn='ci',
                                              method=method)
    lp_scores['splitCommunity-ci_emb'] = splitCommunity_scores_res
    '''
    ##-------------------------------splitter+Community-mi----------------------#
    edgelist_path = '/home/hxm/persona/data/edgelist' + g_name + '.edgelist'
    nx.write_edgelist(g_train, edgelist_path, data=False)
    if lp_scores['splitBIgru_emb']['test_roc'] > lp_scores['splitBIlstm_emb']['test_roc']:
        method = 'BIgru'
    ...
    splitCommunity_scores_res = splitCommunity_scores(g_name, edgelist_path, train_test_split, embedding_dim=embedding_dim,
                                              ...
                                              local_clustering_fn='mi',
                                              method=method)
    lp_scores['splitCommunity-mi_emb'] = splitCommunity_scores_res

    ##-------------------------------splitter+Community-lcm----------------------#
    edgelist_path = '/home/hxm/persona/data/edgelist' + g_name + '.edgelist'
    nx.write_edgelist(g_train, edgelist_path, data=False)
    if lp_scores['splitBIgru_emb']['test_roc'] > lp_scores['splitBIlstm_emb']['test_roc']:
        method = 'BIgru'
    ...
    splitCommunity_scores_res = splitCommunity_scores(g_name, edgelist_path, train_test_split, embedding_dim=embedding_dim,
                                              ...
                                              num_layers=num_layers,
                                              hidden_size=hidden_size*2,
                                              local_clustering_fn='lcm',
                                              method=method)
    lp_scores['splitCommunity-lcm_emb'] = splitCommunity_scores_res

    ##-------------------------------splitter+Community-lci----------------------#
    edgelist_path = '/home/hxm/persona/data/edgelist' + g_name + '.edgelist'
    nx.write_edgelist(g_train, edgelist_path, data=False)
    if lp_scores['splitBIgru_emb']['test_roc'] > lp_scores['splitBIlstm_emb']['test_roc']:
        method = 'BIgru'
    ...
    splitCommunity_scores_res = splitCommunity_scores(g_name, edgelist_path, train_test_split, embedding_dim=embedding_dim,
                                              ...
                                              num_layers=num_layers,
                                              hidden_size=hidden_size*2,
                                              local_clustering_fn='lci',
                                              method=method)
    lp_scores['splitCommunity-lci_emb'] = splitCommunity_scores_res

    ##-------------------------------splitter+Community-lmi----------------------#
    edgelist_path = '/home/hxm/persona/data/edgelist' + g_name + '.edgelist'
    nx.write_edgelist(g_train, edgelist_path, data=False)
    if lp_scores['splitBIgru_emb']['test_roc'] > lp_scores['splitBIlstm_emb']['test_roc']:
        method = 'BIgru'
    ...
    splitCommunity_scores_res = splitCommunity_scores(g_name, edgelist_path, train_test_split, embedding_dim=embedding_dim,
                                              ...
                                              num_layers=num_layers,
                                              hidden_size=hidden_size*2,
                                              local_clustering_fn='lmi',
                                              method=method)
    lp_scores['splitCommunity-lmi_emb'] = splitCommunity_scores_res

    ##-------------------------------splitter+Community-cmi----------------------#
    edgelist_path = '/home/hxm/persona/data/edgelist' + g_name + '.edgelist'
    nx.write_edgelist(g_train, edgelist_path, data=False)
    if lp_scores['splitBIgru_emb']['test_roc'] > lp_scores['splitBIlstm_emb']['test_roc']:
        method = 'BIgru'
    ...
    splitCommunity_scores_res = splitCommunity_scores(g_name, edgelist_path, train_test_split, embedding_dim=embedding_dim,
                                              ...
                                              num_layers=num_layers,
                                              hidden_size=hidden_size*2,
                                              local_clustering_fn='cmi',
                                              method=method)
    lp_scores['splitCommunity-cmi_emb'] = splitCommunity_scores_res
    ...
    if lp_scores['splitBIgru_emb']['test_roc'] > lp_scores['splitBIlstm_emb']['test_roc']:
        ...
        method = 'BIlstm'
    # Using bootstrapped edge embeddings + logistic regression
    splitCommunity2_scores_res = splitCommunity2_scores(g_name, edgelist_path, train_test_split, embedding_dim=embedding_dim,
                                              walk_length=walk_length,
                                              num_walks_node=num_walks_node,
                                              constraint_learning_rate_scaling_factor=constraint_learning_rate_scaling_factor,
                                              iterations=iterations,
                                              ...
                                              num_layers=num_layers,
                                              hidden_size=hidden_size,
                                              local_clustering_fn='ALL',
                                              method=method)
    lp_scores['splitCommunity2_emb'] = splitCommunity2_scores_res


    ##-------------------------------splitter+CommunityMIN----------------------#
    edgelist_path = '/home/hxm/persona/data/edgelist' + g_name + '.edgelist'
    nx.write_edgelist(g_train, edgelist_path, data=False)
    if lp_scores['splitBIgru_emb']['test_roc'] > lp_scores['splitBIlstm_emb']['test_roc']:
        method = 'BIgru'
        ...
        method = 'BIlstm'
    # Using bootstrapped edge embeddings + logistic regression
    splitCommunityMIN_scores_res = splitCommunity_scores(g_name, edgelist_path, train_test_split, embedding_dim=embedding_dim,
                                              walk_length=walk_length,
                                              num_walks_node=num_walks_node,
                                              constraint_learning_rate_scaling_factor=constraint_learning_rate_scaling_factor,
                                              iterations=iterations,
                                              ...
                                              num_layers=num_layers,
                                              hidden_size=hidden_size,
                                              local_clustering_fn='MIN',
                                              method=method)
    lp_scores['splitCommunityMIN_emb'] = splitCommunityMIN_scores_res

    ##-------------------------------splitter+CommunityMAX----------------------#
    edgelist_path = '/home/hxm/persona/data/edgelist' + g_name + '.edgelist'
    nx.write_edgelist(g_train, edgelist_path, data=False)
    if lp_scores['splitBIgru_emb']['test_roc'] > lp_scores['splitBIlstm_emb']['test_roc']:
        method = 'BIgru'
        ...
        method = 'BIlstm'
    # Using bootstrapped edge embeddings + logistic regression
    splitCommunityMAX_scores_res = splitCommunity_scores(g_name, edgelist_path, train_test_split,
                                                         embedding_dim=embedding_dim,
                                                         walk_length=walk_length,
                                                         num_walks_node=num_walks_node,
                                                         constraint_learning_rate_scaling_factor=constraint_learning_rate_scaling_factor,
                                                         ...
                                                         num_layers=num_layers,
                                                         hidden_size=hidden_size,
                                                         local_clustering_fn='MAX',
                                                         method=method)
    lp_scores['splitCommunityMAX_emb'] = splitCommunityMAX_scores_res
    '''
    ### ---------- RETURN RESULTS ---------- ###
    return lp_scores
